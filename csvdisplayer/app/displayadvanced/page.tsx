...
const Home: React.FC = () => {
  ...
  const [calculatedData, setCalculatedData] = useState<CalculatedRow[]>([]); // State for calculated data
  ...
  // Handle input changes for Table 1
  const handleValueChange = (index: number, newValue: string) => {
    // Update the specific row's value
    const updatedData = [...data];
    updatedData[index].Value = newValue;
    setData(updatedData);

    // Recalculate Table 2 data
    const recalculatedData = calculateData(updatedData);
  ...
  return (
    <div className="bg-rose-100 min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4 text-black">
      ...
      <p className="mb-6">
        Note: Changing data from Table 1 will affect data in Table 2 &#x1F92F;
      </p>
      ...
      <div className="pt-10">
        ...
        <table className="min-w-64 bg-white border border-gray-200 mb-10" cellPadding="10" border={1}>
          <thead>
            <tr>
              <th className="border border-black px-4 py-2 text-left text-sm font-medium text-black font-extrabold">
                Category
              </th>
              <th className="border border-black px-4 py-2 text-left text-sm font-medium text-black font-extrabold">
                Value
          ...
          <tbody>
            {calculatedData.map((row, index) => (
              <tr key={index}>
                <td className="border border-black px-4 py-2 text-sm text-black text-left">
                  {row.category}
                </td>
                <td className="border border-black px-4 py-2 text-sm text-black text-left">
                  {row.value}
